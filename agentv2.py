...
# the huber mean loss function for two vectors
...
# suffix Deterministic-v4 means that 4 frames are always skipped
...
class DRLAgent():
	...
	def buildNetwork(numActions, inputShape=(NET_H, NET_W, NET_D)):
		...
		filtered_out = Multiply()([out, action_inp]) # we filter the out layer with action_int layer
	...
	def saveWeights(model):
		DRLAgent.saveWeightsPath(os.path.join(DIRECTORY, SUBDIR, MODEL_NAME_APPEND)+".h5")
	...
	def learn(self, numEpisodes=NUM_EPISODES, observationSteps=OBSERVE_MAX):
		...
		for ep_count in range(numEpisodes):
			...
			while not terminal:
				...
				# 	reward -= 100.0
				self.env.render()
				state = self.get_next_state(state, act, reward, terminal, self.prepState(prev_obs, observation))
			if ep_count % 50 == 0:
	...
	def get_next_state(self, state, action, reward, terminal, envObservation):
