...
TEST_FREQ=1000 # test frequency in parameter updates 
TEST_SET=None
TEST_SET_SIZE=1000
TEST_EPSILON=0.05
# Utility functions
...
class DRLAgent():
	...
	def test(self, numSteps=TEST_STEPS):
		
		# calculate average max Q for fixed states
		if TEST_SET is None:
			printmsg("Test set None!")
			return
		qs = self.qNetwork.predict([TEST_SET, np.ones((len(TEST_SET),self.numActions))], batch_size=len(TEST_SET))
		qs = np.max(qs, axis=1)
		...
		for testEpisode in range(TEST_STEPS): # there will be no more than TEST_STEPS episodes, for sure!
			...
			while not terminal:
				...
				if np.random.rand() < TEST_EPSILON:
					...
					if(USE_TARGET_NETWORK):
						y=self.targetNetwork.predict([np.expand_dims(stackedState, axis=0), np.expand_dims(np.ones(numActions), axis=0)])
					else:
						y=self.qNetwork.predict([np.expand_dims(stackedState, axis=0), np.expand_dims(np.ones(numActions), axis=0)])
	...
	def learn(self, numEpisodes=NUM_EPISODES):
		...
		for self.episodeCount in range(numEpisodes):
			...
			if self.episodeCount % 50 == 0:
				printmsg("Episode {}".format(self.episodeCount))
